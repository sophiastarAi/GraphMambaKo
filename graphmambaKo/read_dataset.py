...
# Read in data
#******************************************************************************
def data_from_name(proj_dir,name, noise = 0.0, theta=2.4):
    if name == 'pendulum_lin':
        return pendulum_lin(noise)      
    if name == 'pendulum':
        return pendulum(noise, theta)
    if name =='PM25':
        train_file = os.path.join(proj_dir,'data/2018_2019_hourly.csv')   #8496,121
        test_file = os.path.join(proj_dir,'data/2018_2019_hourly_test_x.csv')
        val_file = os.path.join(proj_dir,'data/2018_2019_hourly_val_x.csv')
        return PM25(train_file,test_file,val_file)
    else:
...
def PM25(train_file,test_file,val_file):
    train_data = np.loadtxt(train_file, delimiter=',', encoding='utf-8')
    val_data = np.loadtxt(val_file, delimiter=',', encoding='utf-8')
    test_data = np.loadtxt(test_file, delimiter=',', encoding='utf-8')
    return train_data,test_data,val_data,0,0,train_data.shape[1],1
